    ...
    res.send(`
        ...
        <head>
            ...
            <style>
                ...
                textarea { width: 90%; height: 150px; background: #2a2a2a; border: 1px solid #444; color: #00ff00; font-family: monospace; padding: 10px; }
                button { padding: 12px 20px; margin: 5px; border: none; border-radius: 6px; cursor: pointer; font-weight: bold; background: #28a745; color: white; }
                input { padding: 10px; background: #2a2a2a; border: 1px solid #444; color: white; margin-bottom: 10px; }
            </style>
        </head>
        <body>
            <h1>Roblox Remote Executor</h1>
            
            <div class="section">
                <h3>Whitelist</h3>
                <input type="text" id="selfName" placeholder="Username...">
                <button onclick="joinWhitelist()">Add to Whitelist</button>
            </div>

            <div class="section">
                <h3>Custom Lua Script</h3>
                <textarea id="luaCode" placeholder="-- Example:\\ngame.Lighting.ClockTime = 0"></textarea><br>
                <button onclick="sendLua()">Execute Script (Loadstring)</button>
            </div>
            ...
            <script>
                ...
                async function sendLua() {
                    ...
                    const res = await fetch('/send-command', {
                        method: 'POST',
                        headers: {'Content-Type': 'application/json'},
                        body: JSON.stringify({ cmd: 'CustomScript', key: "${SECRET_KEY}", script: script })
                    });
                    if(res.ok) alert("Script Sent to Roblox!");
                }
            </script>
        </body>
        </html>
...
app.post('/send-command', (req, res) => {
    const { cmd, key, script } = req.body;
    if (key === SECRET_KEY) {
        pendingCommand = { command: cmd, script: script };
        res.json({ success: true });
    } else {
